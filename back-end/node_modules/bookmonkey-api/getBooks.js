...
/* async function updateBook(bookIsbn, updatedData) {
  const response = await fetch(`https://api.itbook.store/1.0/books/${bookIsbn}`);
  const book = await response.json();
  ...
  const updateResponse = await fetch(`https://api.itbook.store/1.0/books/${bookIsbn}`, {
    method: 'PUT',
    headers: {
...
/* async function deleteBook(bookIsbn) {
  ...
  const deleteResponse = await fetch(`https://api.itbook.store/1.0/books/${bookIsbn}`, {
    method: 'DELETE',
  });
...
/* (async function() {
  const updatedBook = await updateBook('isbn', { title: 'New Title' });
  console.log('Updated Book:', updatedBook);

  const deletedBook = await deleteBook('isbn');
